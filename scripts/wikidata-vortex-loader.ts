#!/usr/bin/env npx tsx
/**
 * Wikidata Vortex Loader
 *
 * Streams the full Wikidata JSON dump and outputs Vortex files with:
...
const TEMP_DIR = '/tmp/wikidata-vortex-loader';
...
async function writeVortexChunk(triples: Triple[], chunkIndex: number): Promise<{ path: string; size: number }> {
  const chunkId = `chunk_${chunkIndex.toString().padStart(6, '0')}`;
  const jsonlPath = join(TEMP_DIR, `${chunkId}.jsonl`);
  const vortexPath = join(TEMP_DIR, `${chunkId}.vortex`);
  const r2Path = `datasets/wikidata-vortex/chunks/${chunkId}.vortex`;
  ...
  // Use DuckDB to convert to Vortex with sorting
...
`;
  ...
  // Get file size
  const { size } = statSync(vortexPath);

  // Upload to R2
  execSync(`npx wrangler r2 object put ${R2_BUCKET}/${r2Path} --file=${vortexPath} --content-type=application/octet-stream`, {
    stdio: 'pipe',
    cwd: join(__dirname, '..'),
  });
...
async function loadWikidataVortex(maxEntities: number = Infinity): Promise<void> {
  console.log('📚 Wikidata Vortex Loader');
  console.log('=========================');
  ...
  console.log(`Output: Vortex with full Wikidata types`);
  console.log('');
  ...
  const flushChunk = async () => {
    ...
    try {
      const { size } = await writeVortexChunk(triples, chunkIndex);
      totalBytes += size;
      console.log(`      → ${(size / 1024 / 1024).toFixed(2)}MB vortex`);
    } catch (err: any) {
  ...
  execSync(`npx wrangler r2 object put ${R2_BUCKET}/datasets/wikidata-vortex/manifest.json --file=${manifestPath} --content-type=application/json`, {
    stdio: 'pipe',
    cwd: join(__dirname, '..'),
  });
  ...
  console.log('');
  console.log('✅ Completed Wikidata Vortex load:');
...
loadWikidataVortex(maxEntities).catch(console.error);
